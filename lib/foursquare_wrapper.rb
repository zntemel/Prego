class FoursquareWrapper
  ...
  def self.get_venues_for(city)
    url = API_URL + "?near=#{city.name}&oauth_token=#{API_KEY}&v=20161217"

    data = HTTParty.get(url)

    city_pricing = create_avarage_pricing_for(city)
    city_rating = create_avarage_rating_for(city)
    city_visit_frequency = create_avarage_visit_frequency_for(city)
    save_venue_pricing_for_city(data, city_pricing, city_rating, city_visit_frequency)
  end

  def self.save_venue_pricing_for_city(data, city_pricing, city_rating, city_visit_frequency)
    data['response']['groups'].each do |group|
      if group.present? && group['items'].present?
        group['items'].each do |item|
          if item.present? && item['venue'].present?
            venue = Venue.new
            venue.city_pricing = city_pricing
            venue.city_rating = city_rating
            venue.city_visit_frequency = city_visit_frequency
            if item['venue']['price'].present? && item['venue']['price']['message'].present?
              venue.pricing = item['venue']['price']['tier']
              venue.save
            end
  ...
  def self.create_avarage_pricing_for(city)
    CityPricing.create(city: city)
  end
